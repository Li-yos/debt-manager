...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function apiFetch(endpoint, options = {}) { /* ... 和之前一样 ... */ }
    async function fetchAndRenderDebtors() { /* ... 和之前一样 ... */ }
    async function fetchAndRenderItems(debtorId) { /* ... 和之前一样 ... */ }
    function handleSelectDebtor(liElement) { /* ... 和之前一样 ... */ }
    async function editDebtor(debtorId, newName, contactInfo = null) { /* ... 和之前一样 ... */ }
    async function recordPayment(debtorId, amount) { /* ... 和之前一样 ... */ }
    
    // 【修改点】获取并渲染还款历史的函数
    async function fetchAndRenderPayments(debtorId) {
        try {
            const response = await apiFetch(`/payments?debtorId=${debtorId}`);
            if (!response || !response.ok) {
                paymentsHistoryToggle.classList.add('hidden');
            ...
            if (payments.length > 0) {
                // 如果有还款历史，显示标题
                paymentsHistoryToggle.classList.remove('hidden');
                payments.forEach(payment => {
                    const li = document.createElement('li');
                    li.style.backgroundColor = '#f0fdf4';
                    li.innerHTML = `<div class="item-main"><div>还款 <strong>¥${payment.amount.toFixed(2)}</strong></div><div class="item-meta">日期: ${new Date(payment.payment_date).toLocaleDateString()}</div></div>`;
    ...
    // ... (debtorsList, addDebtorForm, addItemForm, debtItemsList, repayBtn 的事件监听器都保持不变) ...

    // 【新增】为还款历史标题添加点击事件
    paymentsHistoryToggle.addEventListener('click', () => {
        const icon = paymentsHistoryToggle.querySelector('.toggle-icon');
        // 为列表和图标切换 'collapsed' 类
        paymentsHistoryList.classList.toggle('collapsed');
