...
export async function onRequest(context) {
    try {
        ...
        if (error.message?.includes('UNIQUE constraint failed')) {
            return new Response(JSON.stringify({ error: 'A debtor with this name already exists.' }), { status: 409 });
        }
        console.error('An error occurred in debtors.js:', error);
